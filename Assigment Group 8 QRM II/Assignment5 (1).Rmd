...
This dataset has 606 observations. There are 19 variables and 606 subjects. what consists of a subject...
...
The mean of profits is $55265374. The median of profits is 0. The minimum of profits is -111007242. The maximum of profits is 1316249360.

*Step b*

```{r}
which.max(movies2$profits)
...
library(ggplot2)
ggplot(movies2, aes(x = , y = profits)) + 
  geom_boxplot()
...
For the negative profits you get a NaN, not a number, error. If we take the mean of the log of the profits and remove the NA's then we get -Infnity.
